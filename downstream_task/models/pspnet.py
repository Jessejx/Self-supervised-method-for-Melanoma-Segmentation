...
def resnet50(drop_rate=0, pretrained=True):
    model = ResNet(Bottleneck, [3, 4, 6, 3], drop_rate=drop_rate)
    if pretrained:
        state_dict = torch.load("/media/tzh/data_1/workings_ljx/Melanoma_Segmentation/weights/checkpoint_cg_240.pth")
        ...
        # state_dict = state_dict['trunk']
        # state_dict = {k.replace('_feature_blocks.', ''): v for k, v in state_dict.items()}
...
class PSPNet(nn.Module):
    def __init__(self, n_classes=2, sizes=(1, 2, 3, 6), psp_size=2048,
        ...
        if backend =='resnet18':
            ...
            self.encoder = resnet50(drop_rate)
        else:
